...
for (const lang of ['ko', 'es', 'en']) {
  for (const page of ['index', 'about', 'products', 'qna', 'contact']) {
    ...
    assert(doc.querySelector('main').textContent.trim().length > 300);
...
console.log('SEO regressions passed: 15 complete initial documents, real content, links, form options, no pre-bound animations.');
